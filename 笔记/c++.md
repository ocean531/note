...
###### final关键字
在继承了一个类之后，子类可以进行重写，如果不希望子类的子类再次进行重写，那么可以用final关键字，这时就禁止了再次重写
```C++
...
##### 对引用&的看法
函数接受变量，有的只是为了在函数中使用该变量，但是有一些函数的功能是修改被传入的变量，这时就要用到引用，因为正常的传入是将该变量拷贝了一份给函数，而用引用则是直接操作变量的值，更深入的讲，在函数不需要修改变量时，可以用 const只读修饰符加上引用&，对于一些大型的对象来说会更快，特别是结构体，但是对于一些比较小的类型，直接传入可能更快，因为引用的底层是指针，会产生一些而外的开销
...
```
但是但是断言是在运行时发生，也就是说上面的函数只有在调用时才会发生断言
在C++11中，新添加了静态断言
#### 静态断言
静态断言就是在编译时就进行检查，使用时不需要引用头文件，而且也可以自定义违反断言时的错误提示信息
```
static_assert(sizeof(long) == 4, "错误, 不是32位平台...");
```
接受两个参数，断言表达式和警告信息
由于静态表达式是在编译阶段进行检测，所以在他的表达时中==不可以出现变量==，必须是常量表达式

### 左右值
#### 概念
- 左值是值存储在内存中、有明确存储地址的数据，可取地址
...
a = b;
```
上面的例子中，a和b都是左值，10和20都是右值
C++11之后，支持右值引用
右值引用就是告诉编译器这个变量可以直接浅拷贝
#### 移动构造
右值引用可以实现移动构造
```
 MyString(MyString&& other) noexcept {
        data_ = other.data_;   // 直接把指针拿过来
        size_ = other.size_;
        other.data_ = nullptr; // 原来的对象置空，防止析构时重复释放
        other.size_ = 0;
    }
```
以std::string为例，
...
suto b = std::move(a);//移动构造，将a的所有转移给b，效率更高，但是a会被清空
```
std::move()，将传入的变量转为右值引用